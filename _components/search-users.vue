	...
	export default {
		...
		methods: {
			...
			getUsers(page, done) {
				...
				//Request users
				this.$crud.index('apiRoutes.quser.users', {params: params}).then(response => {
					setTimeout(() => {
			...
			//Get Departments
			getDepartments() {
				this.$crud.index('apiRoutes.quser.departments').then(response => {
			...
			//Get Roles
			getRoles() {
				this.$crud.index('apiRoutes.quser.roles').then(response => {
